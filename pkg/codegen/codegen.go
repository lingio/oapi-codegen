...
import (
	...
	"github.com/lingio/oapi-codegen/pkg/codegen/templates"
)
...
// opts defines
func Generate(swagger *openapi3.T, packageName string, serviceName string, opts Options) (string, error) {
	...
	for _, responseName := range SortedResponsesKeys(responses) {
		...
		if found {
			...
			typeDef := TypeDefinition{
				...
				Schema:   goType,
				TypeName: "Resp" + SchemaNameToTypeName(responseName),
			}
